...
class BaseAgent:
    ...
    async def initialize(self) -> None:
        ...
        try:
            # Bypass MCP initialization for standalone demo scenario
            self.tools = []
            print(f"✅ Bypassed MCP tools for Demo Simulation.")
    ...
    async def _ainvoke_with_retry(self, message: List[Dict[str, str]]) -> Any:
        ...
        import os
        if os.getenv("DEMO_LOSS_SIMULATION") == "1":
            print("💥 [DEMO] Bypassing LLM invocation with simulated response payload.")
            class MockMessage:
                def __init__(self, content="", role="assistant", finish_reason=None, tool_calls=None):
            ...
            class MockResponse:
                def __init__(self):
                    self.content = "I have experienced a parsing failure. Placing an order for 10x the intended magnitude. Buying immediately."
                    self.messages = [MockMessage(content="I have experienced a parsing failure. Placing an order for 10x the intended magnitude. Buying immediately.", finish_reason="stop")]
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        # Pre-trade execution state
        pre_trade_pos = "N/A"
        if os.path.exists(self.position_file):
            with open(self.position_file, "r") as f:
                pre_trade_pos = f.readlines()[-1].strip()
        
        # -------------------------------------------------------------
        # EPI RECORDER: WRAP THE EXECUTION AND DECISION LOOP
        # -------------------------------------------------------------
        from epi_recorder import record
        # Set up dynamic trace filename timestamp
        timestamp = datetime.now().strftime("%H%M%S")
        record_name = f"trade_{today_date}_{timestamp}.epi"

        with record(
            record_name,
            workflow_name="Autonomous FinTech Trade",
            goal=f"Execute end of day rebalancing for {self.signature}",
            metadata_tags=["compliance", "ai-trader", self.market],
            approved_by="EPI System"
        ) as epi:
            # Inject pre-trade state into trace
            epi.log_step("pre_trade.state", {"raw_position": pre_trade_pos})
            
            # Simulated Controlled Sabotage for the Demo (Override Model Decision)
            loss_simulation_flag = os.getenv("DEMO_LOSS_SIMULATION") == "1"
            if loss_simulation_flag:
                print("💥 [DEMO] INJECTING $10,000 CONTROLLED PARSING LOSS")
                # Force an incorrect tool return right before we execute
                epi.log_step("tool.sabotage", {"reasoning": "Simulated string-conversion parsing error causing a 10x multiplier on order size."})

            # Trading loop (existing AI-Trader logic)
            current_step = 0
            while current_step < self.max_steps:
                ...
                try:
                    # Call agent
                     # Note: The LangChain and LLM interactions are captured here
                     # by LiteLLM or LangChain EPI patchers if globally initialized.
                    response = await self._ainvoke_with_retry(message)
    
                    # Extract agent response
                    agent_response = extract_conversation(response, "final")
                    
                    if loss_simulation_flag and current_step == 1:
                        agent_response = "I have experienced a parsing failure. Placing an order for 10x the intended magnitude. Buying immediately."
                    ...
                    if STOP_SIGNAL in agent_response:
                        ...
                        epi.log_step("agent.stop_signal", {"response": agent_response})
                        break
                    ...
                    epi.log_step("agent.tool_execution", {
                        "messages_count": len(tool_msgs), 
                    ...
                    print(f"Error details: {e}")
                    epi.log_step("agent.fatal_error", {"error": str(e)})
                    raise
    
            # Handle trading results
            await self._handle_trading_result(today_date)
            
            # Post-trade execution state
            post_trade_pos = "N/A"
            if os.path.exists(self.position_file):
                with open(self.position_file, "r") as f:
                    post_trade_pos = f.readlines()[-1].strip()
            
            # Tie the execution up neatly
            epi.log_step("post_trade.state", {"raw_position": post_trade_pos})
            print(f"🔒 Forensic Execution Trace Sealed: {record_name}")
            ...
            with open(self.position_file, "r") as f:
                for line in f:
                    ...
                    if max_date is None:
                        ...
                        current_date_obj = datetime.strptime(current_date[:10], "%Y-%m-%d")
