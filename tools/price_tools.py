...
def get_yesterday_date(today_date: str, merged_path: Optional[str] = None, market: str = "us") -> str:
    ...
    # 解析输入日期/时间
    if ' ' in today_date:
        # Sometimes the format might have unconverted data, handle strictly to the lengths
        input_dt = datetime.strptime(today_date[:19], "%Y-%m-%d %H:%M:%S")
        date_only = False
    else:
        input_dt = datetime.strptime(today_date[:10], "%Y-%m-%d")
        date_only = True
